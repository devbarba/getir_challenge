...
describe('Testing error handling functions', () => {
    ...
    test('expect return error calling Handler', () => {
        expect(() => {
            throw new Handler('handling error for testing purpouses', 10001, NOT_FOUND)
        })
    ...
    test('expect return error other errors when call Handler.getErrors()', () => {
        const error = new Handler('getting other errors', 1004, PRECONDITION_FAILED);

        error.setErrors(['Unexpected token } in JSON at position 2022']);
        expect(error.getErrors()).toEqual(['Unexpected token } in JSON at position 2022']);
        expect.assertions(1);
    });

    test('expect return error code when call Handler.getCode()', () => {
        const error = new Handler('getting other errors', 1005, PERMANENT_REDIRECT);
